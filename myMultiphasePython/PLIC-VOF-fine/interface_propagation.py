...
# 界面推进，先计算体积通量CL、CR和CS，再沿x方向和y方向推进C
    ...
    for j in range(0, len(y)):
        for i in range(0, len(x)):
            ...
            if C[j, i] > 1.0 - minima:  # C=1的情况
                ...
                if flag:
                    CL[j, i], CR[j, i] = CR[j, i], CL[j, i]  # 当(nx, ny)不在第一象限时进行交换CL和CR


    #myplot.plotContour(x, y, CS)
    #myplot.plotContour(x,y,CR)
    Cstar[1:-1, 1:-1] = CS[1:-1, 1:-1] + CR[1:-1, 0:-2] + CL[1:-1, 2:]  # 更新C
    Cstar = np.minimum(1.0, np.maximum(0.0, Cstar))  # 清理大于1或小于0的C
